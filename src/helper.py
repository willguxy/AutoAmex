...
def get_driver(browser):
    ...
    if browser.lower() == 'firefox':
        ...
        try:
            ...
            logging.error(
                "Error: An unexpected error occurred while initializing the Chrome driver.")
            sys.exit(1)
        ...
        logging.warning(
            "WARNING: browser selection not valid, use Chrome as default")
        driver = Chrome(options=options)
    return driver
...
def click_on_offers_on_page(driver):
    ...
    if offers:
        logging.info("Offers on this page: %s." %
                     offers.split('account ending')[0])
    else:
        logging.info("No offers on this page.")
    for _ in range(3):
        if not collect_offer_names(driver):
            return
    ...
    while True:
        switcher_toggle_button = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable(
                (By.CSS_SELECTOR, ".flex.btn.axp-account-switcher__accountSwitcher__togglerButton___1H_zk.account-switcher-toggler.css-15ld01r"))
        )
        switcher_toggle_button.click()
        WebDriverWait(driver, 10).until(
            EC.visibility_of_element_located((By.ID, "switcher_product_rows"))
        )
        all_card_buttons = driver.find_elements(
            By.CSS_SELECTOR, "#switcher_product_rows button")
        card_buttons = [
            button for button in all_card_buttons if 'ending in' in button.accessible_name and button.accessible_name not in visited_cards]
        if not card_buttons:
            break
        button = card_buttons[0]
        logging.info("Clicking on card: %s" %
                     button.accessible_name.split(' account')[0])
